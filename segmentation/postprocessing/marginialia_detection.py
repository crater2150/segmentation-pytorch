...
def marginalia_detection(bboxs: List[BboxCluster], image, num_border_threshold=5, min_border_length=150,
                         max_gap_length=15, min_segment_length = 10):
    change = False
    left_borders = []
    right_borders = []
    ...
    for x in l_borders.keys():
        ...
        for y in r_borders.keys():
            ...
            if med <= max_gap_length:
                ...
                print(2)
                print(min_y_l, max_y_l, min_y_r, max_y_r)
                if not (max_y_l < min_y_r or max_y_r < min_y_l) and not bool(l_id & r_id):
                    print(1)
                    # print("Mean, {} , Med, {}".format(mean, med))
                    keys_to_remove.append((y, x))
                    l1 = len(l_borders[x])
                    l2 = len(r_borders[y])
                    # if l1 > l2:
                    #    border_ = [(l[0] - med, l[1]) for l in l_borders[x]]
                    #    top_ = [(border_[0][0], l[1]) for l in r_borders[x] if l[1] < min_y_l]
                    #    bot_ = [(border_[-1][0], l[1]) for l in r_borders[x] if l[1] > max_y_l]
                    #    border_ = top_ + border_ + bot_
                    #    pass
                    # if l2 > l1:
                    #    border_ = [(l[0] - med, l[1]) for l in r_borders[x]]
                    #    top_ = [(border_[0][0], l[1]) for l in l_borders[x] if l[1] < min_y_l]
                    #    bot_ = [(border_[-1][0], l[1]) for l in l_borders[x] if l[1] > max_y_l]
                    #    border_ = top_ + border_ + bot_
                    #    pass
                    ...
                    border_dict[x] = [(x[0], x[1]) for x in border_line]
                    # border[x] = b
                            ...
                            for ind2, point in enumerate(baseline.baseline):
                                if point[0] > x:
                                    if not (len(baseline.baseline[:ind2 - 1]) < 5 or len(baseline.baseline[ind2:]) < 5):
                                        baselines.append(
                                            BaselineResult(baseline.baseline[:ind2 - 1], height=baseline.height,
                                                           font_width=baseline.font_width,
                                                           cluster_type=baseline.cluster_type,
                                                           cluster_location=baseline.cluster_location))
                                        baselines.append(
                                            BaselineResult(baseline.baseline[ind2:], height=baseline.height,
                                                           font_width=baseline.font_width,
                                                           cluster_type=baseline.cluster_type,
                                                           cluster_location=baseline.cluster_location))
                                    else:
